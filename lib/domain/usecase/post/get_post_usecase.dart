...
class GetPostUseCase extends UseCase {
  ...
  Future<PostDTO?> execute(String postId) async {
    var uid = _authService.getUid(token);
    if (null == uid) {
      throw Exception('invalid token');
    }
    var data = await _postRepository.getPost(postId);
    if (null == data) {
      return null;
    }
    ...
    var userLikedPosts =
        (await _likedPostRepository.queryListByUid(user.uid))
            .map((e) => e.postId)
            .toList();
    var userFavoritePosts =
        (await _favoritePostRepository.queryListByUid(user.uid))
            .map((e) => e.postId)
            .toList();
    var postDTO = await _toPostDTO(data, userLikedPosts, userFavoritePosts);
    return postDTO;
  }

  Future<PostDTO> _toPostDTO(Post post, List<String> userLikedPosts,
      List<String> userFavoritePosts) async {
    List<LikedPost> likedPostList =
        await _likedPostRepository.queryListByPostId(post.id);
    var likeCount = likedPostList.length;

    var like = userLikedPosts.contains(post.id);
    var favorite = userFavoritePosts.contains(post.id);
